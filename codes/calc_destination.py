...
def calculate_trajectory(site_lon,site_lat,lon_flow,lat_flow,uv_data,total_time_in_hr,step_in_hr):
...
if __name__=='__main__':
  ...
  #for run in ['historical',]:
    ...
    for site in lonlat.keys():
      ...
      df['%s_U'%site]=site_flow[:,0]
      df['%s_V'%site]=site_flow[:,1]
      df['%s_WD'%site]=[uv2deg(uu,vv) for uu,vv in zip(site_flow[:,0],site_flow[:,1])]
      df['%s_WS'%site]=np.sqrt(site_flow[:,0]*site_flow[:,0]+site_flow[:,1]*site_flow[:,1])
    ...
    for site in['TPP','SNC']:
      ...
      #for idx in [29,]:
        ...
        traj=calculate_trajectory(lonlat[site][0],lonlat[site][1],\
      ...
      df['%s_dest_lon'%site]=dest_lon
      df['%s_dest_lat'%site]=dest_lat
      fname=f'../data/traj.{site}.{m}_{runDict[run][0]}_{runDict[run][1]:04d}0101-{runDict[run][2]:04d}1231.npy'
      np.save(fname,np.array(traj_list))
    df.to_csv('../data/sites.%s_%s_%04d0101-%04d1231.csv'%\
