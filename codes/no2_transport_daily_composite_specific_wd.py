...
if __name__=='__main__':
    ...
    for wd_cat in [60,90,120,150,180]:
        ...
        for ax,run in zip(axes.ravel(),['2025','2085']):
            ...
            composite=no2Dict[run][idxList,:,:].mean(axis=0)
            uv=flowDict[run][idxList,:,:,:].mean(axis=0)
            ...
            strm,cf_no2=plot_no2(ax,data,lonTW,latTW,topo,\
              np.linspace(0,300,16),r_cmap,f'{runDict[run][1]}-{runDict[run][2]}\n({idxList.shape[0]} days)')
        ...
        plt.suptitle(f'NO2 Distribution Composite (WD={wd_cat})',fontsize=15)
        plt.savefig(f'../figures/no2_daily_comp.wd{wd_cat:03}.png')
